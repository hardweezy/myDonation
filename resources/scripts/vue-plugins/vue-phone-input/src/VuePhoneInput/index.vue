<template>
  <div
    class="intl-phone-input allow-dropdown input is-default"
    ...
    >
      ...
      <ul class="country-list" v-if="isVisiblePanel">
        <li
          ...
          v-bind:class="`country${item.code === countryCode ? ' highlights' : ''}`"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      maxLength(){
        var checkLength = this.intlData.example.replace(/ /g, '');
